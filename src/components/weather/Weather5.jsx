...
export const Weather5 = () => {
  ...
  const [min, setMin] = useState(
    () => localStorage.getItem(`${PREFIX}_min`) || '9',
  );
  const [max, setMax] = useState(
    () => localStorage.getItem(`${PREFIX}_max`) || '17',
  );
  const [city, setCity] = useState(
    () => localStorage.getItem(`${PREFIX}_city`) || 'CANGUÇU',
  );
  const [icon, setIcon] = useState(
    () => localStorage.getItem(`${PREFIX}_icon`) || 'sunny',
  );

  const increaseMin = () => {
    const value = Number(min) + 1;
    setMin(String(value));
    localStorage.setItem(`${PREFIX}_min`, String(value));
  };
  ...
  const increaseMax = () => {
    ...
    setMax(String(value));
    localStorage.setItem(`${PREFIX}_max`, String(value));
  };
  ...
  const handleCity = (value) => {
    ...
    setCity(v);
    localStorage.setItem(`${PREFIX}_city`, v);
  };

  const handleIcon = (value) => {
    setIcon(value);
    localStorage.setItem(`${PREFIX}_icon`, value);
    setShowIcons(false);
  };

...
export const Weather6 = () => {
  ...
  const [min, setMin] = useState(
    () => localStorage.getItem(`${PREFIX}_min-2`) || '9',
  );
  const [max, setMax] = useState(
    () => localStorage.getItem(`${PREFIX}_max-2`) || '17',
  );
  const [city, setCity] = useState(
    () => localStorage.getItem(`${PREFIX}_city-2`) || 'SÃO LOURENÇO',
  );
  const [icon, setIcon] = useState(
    () => localStorage.getItem(`${PREFIX}_icon-2`) || 'sunny',
  );

  const increaseMin = () => {
    const value = Number(min) + 1;
    setMin(String(value));
    localStorage.setItem(`${PREFIX}_min-2`, String(value));
  };
  ...
  const increaseMax = () => {
    ...
    setMax(String(value));
    localStorage.setItem(`${PREFIX}_max-2`, String(value));
  };
  ...
  const handleCity = (value) => {
    ...
    setCity(v);
    localStorage.setItem(`${PREFIX}_city-2`, v);
  };

  const handleIcon = (value) => {
    setIcon(value);
    localStorage.setItem(`${PREFIX}_icon-2`, value);
    setShowIcons(false);
  };
